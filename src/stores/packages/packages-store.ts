...
export class PackagesStore {
  ...
  async init(): Promise<void> {
    try {
      const response = await fetch(
        `${window.env.SBER_DPC_HOST}/api/qmobile_catalog/products/Kit?fields=Id,Modifiers,MarketingProduct.Title,MarketingProduct.Description,MarketingProduct.KitType,Images`,
      );
  ...
  async fetchPackage(packageId: string): Promise<void> {
    try {
      const response = await fetch(
        `${window.env.SBER_DPC_HOST}/api/qmobile_catalog/products/${packageId}`,
      );
      ...
      for (const product of this._currentPackage.ProductsInKit) {
        ...
        const itemResponse = await fetch(
          `${window.env.SBER_DPC_HOST}/api/qmobile_catalog/products/${product.Id}`,
        );
