...
export class DevicesStore {
  ...
  async init(): Promise<void> {
    try {
      const response = await fetch(
        `${window.env.DPC_HOST}/api/qmobile_catalog/products/Device?fields=MarketingProduct,Parameters,Id,Images,Modifiers`,
      );
  ...
  async fetchDevice(deviceId: string): Promise<void> {
    try {
      const response = await fetch(
        `${window.env.DPC_HOST}/api/qmobile_catalog/products/${deviceId}`,
      );
